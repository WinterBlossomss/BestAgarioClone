const {isVisibleEntity} = require("../util/entityutil");
...
exports.Map = class {
    ...
    enumerateWhatPlayersSee(callback) {
        for (let currentPlayer of this.players.data) {
            let visibleFood = this.food.data.filter(entity => isVisibleEntity(entity, currentPlayer, false));
            let visibleViruses = this.viruses.data.filter(entity => isVisibleEntity(entity, currentPlayer));
            let visibleMass = this.massFood.data.filter(entity => isVisibleEntity(entity, currentPlayer));
